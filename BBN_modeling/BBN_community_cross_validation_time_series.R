...
load(paste0(local_path, "merged_community_expand.rda"))

# Also read in the interpolated data for individual risk prediction later
estimated_infection_race = read.csv(paste0(local_path, "race-ethnicity_interpolated.csv")) # For each race category, the proportion of infected people in that category
estimated_infection_age = read.csv(paste0(local_path, "AgeInterpolationFinalDF.csv")) # For each age category, the proportion of infected people in that category
...
save(d_bins, file = (paste0(local_path, "discretization_params.rda")))
