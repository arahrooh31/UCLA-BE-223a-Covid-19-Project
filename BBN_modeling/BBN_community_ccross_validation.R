...
cleaned_data = 
  ...
  select(outbreak_3d_lead, cumulative_cases, new_cases_today, high_school_orless)
...
structure = empty.graph(c("outbreak_3d_lead", "cumulative_cases", "new_cases_today", "high_school_orless"))
modelstring(structure) = paste0("[outbreak_3d_lead|cumulative_cases:new_cases_today][new_cases_today|cumulative_cases][cumulative_cases|high_school_orless][high_school_orless]")
...
for (fold_index in 1:num_folds){
  ...
  if (fold_index == 5){
    save(fold_testing_data, file = paste0(local_path, "fold_testing_data.rda"))
  }
  ...
  print(paste("The sensitivity of the fold is", round(sens_spec_fold$byClass["Sensitivity"], 3)))
  print(paste("The specificity of the fold is", round(sens_spec_fold$byClass["Specificity"], 3)))
  print(paste("The F1 score of the fold is", round(sens_spec_fold$byClass["F1"], 3)))
...
save_place_ID_and_outbreak_risk = data.frame(merged_community$place_ID[as.numeric(rownames(cleaned_data_no_nan))], pred_positive_class_prob)
