...
make_prediction_on_all_communities = function(filepath){
  ...
  community_latest = 
    merged_community_expand  %>% 
    filter(date == max(as.Date(merged_community_expand$date)))   # Get the latest day available
  ...
  community_latest = as.data.frame(community_latest)
  ...
  save_place_ID_and_outbreak_risk = 
    ...
    mutate(community_pred_test_samples_category = as.factor(case_when(
      community_pred_test_samples_prob < 0.5  ~ "No_outbreak",
      TRUE ~ "Yes_outbreak"
    ))) %>%
...
make_prediction_on_one_person = function(community, age, race){
  ...
  population_age_category = c(2172172, 4584585, 1921922, 1321321) # In sequence of age_18_less, 19-49, 50-64, 65_up
  infected_age_category = c(32015, 182513, 59141, 33533) # In sequence of age_18_less, 19-49, 50-64, 65_up
  ...
  infAge = c(infAge_conditional[1] * 0.217217, infAge_conditional[2] * 0.458458, infAge_conditional[3] * 0.192192, infAge_conditional[4] * 0.132132) / p_covid_incidence_rate
  not_infAge = 1 - infAge # Proportion of people who didn't get COVID within each specific age category using LA county data
  
  population_in_race = c(1546022, 903519, 4879006, 2620206) # In sequence of Asian, Black, Latino, White
  infected_in_race = c(10473, 10113, 129999, 28990) # In sequence of Asian, Black, Latino, White
  ...
  infRace = c(infRace_conditional[1] * 0.154, infRace_conditional[2] * 0.09, infRace_conditional[3] * 0.486, infRace_conditional[4] * 0.261) / p_covid_incidence_rate
  not_infRace = 1 - infRace # Proportion of people who didn't get COVID within each specific race category using LA county data
  
  infOutbreak = c(individual_model_outbreak_infection$crude_rate_7d_lead$prob["Yes_infection", "No_outbreak"] , individual_model_outbreak_infection$crude_rate_7d_lead$prob["Yes_infection", "Yes_outbreak"]) # In sequence of No_outbreak, Yes_outbreak. Numbers are from automatically learned CPT.
  ...
  # Priors 
  cpt_age = array(c(0.217217, 0.458458, 0.192192, 0.132132), dim = 4, dimnames = list("age" = c("age_18_less", "age_19_o_49", "age_50_to_64", "age_65_up"))) # The probability of each age category
  cpt_race = array(c(0.154, 0.09, 0.486, 0.261), dim = 4, dimnames = list("race" = c("Asian", "Black", "Latino", "White"))) # The probability of each race category
  cpt_outbreak = array(individual_model_outbreak_infection$pred_outbreak_risk$prob, dim = 2, dimnames = list("outbreak" = c("No_outbreak", "Yes_outbreak"))) # The probability of each outbreak risk category
  ...
  cpt_age_race_outbreak_infection = array(c(infOutbreak[1] * infAge[1] * infRace[1], 1 - (infOutbreak[1] * infAge[1] * infRace[1]), infOutbreak[1] *infAge[1] * infRace[2], 1 - (infOutbreak[1] *infAge[1] * infRace[2]), infOutbreak[1] *infAge[1] * infRace[3], 1 - (infOutbreak[1] *infAge[1] * infRace[3]), infOutbreak[1] *infAge[1] * infRace[4], 1 - (infOutbreak[1] *infAge[1] * infRace[4]), 
                                            infOutbreak[1] *infAge[2] * infRace[1], 1 - (infOutbreak[1] *infAge[2] * infRace[1]), infOutbreak[1] *infAge[2] * infRace[2], 1 - (infOutbreak[1] *infAge[2] * infRace[2]), infOutbreak[1] *infAge[2] * infRace[3], 1 - (infOutbreak[1] *infAge[2] * infRace[3]), infOutbreak[1] *infAge[2] * infRace[4], 1 - (infOutbreak[1] *infAge[2] * infRace[4]),
                                            infOutbreak[1] *infAge[3] * infRace[1], 1 - (infOutbreak[1] *infAge[3] * infRace[1]), infOutbreak[1] *infAge[3] * infRace[2], 1 - (infOutbreak[1] *infAge[3] * infRace[2]), infOutbreak[1] *infAge[3] * infRace[3], 1 - (infOutbreak[1] *infAge[3] * infRace[3]), infOutbreak[1] *infAge[3] * infRace[4], 1 - (infOutbreak[1] *infAge[3] * infRace[4]), 
                                            infOutbreak[1] *infAge[4] * infRace[1], 1 - (infOutbreak[1] *infAge[4] * infRace[1]), infOutbreak[1] *infAge[4] * infRace[2], 1 - (infOutbreak[1] *infAge[4] * infRace[2]), infOutbreak[1] *infAge[4] * infRace[3], 1 - (infOutbreak[1] *infAge[4] * infRace[3]), infOutbreak[1] *infAge[4] * infRace[4],1 - (infOutbreak[1] *infAge[4] * infRace[4]), 
                                            
                                            infOutbreak[2] * infAge[1] * infRace[1], 1 - (infOutbreak[2] * infAge[1] * infRace[1]), infOutbreak[2] *infAge[1] * infRace[2], 1 - (infOutbreak[2] *infAge[1] * infRace[2]), infOutbreak[2] *infAge[1] * infRace[3], 1 - (infOutbreak[2] *infAge[1] * infRace[3]), infOutbreak[2] *infAge[1] * infRace[4], 1 - (infOutbreak[2] *infAge[1] * infRace[4]), 
                                            infOutbreak[2] *infAge[2] * infRace[1], 1 - (infOutbreak[2] *infAge[2] * infRace[1]), infOutbreak[2] *infAge[2] * infRace[2], 1 - (infOutbreak[2] *infAge[2] * infRace[2]), infOutbreak[2] *infAge[2] * infRace[3], 1 - (infOutbreak[2] *infAge[2] * infRace[3]), infOutbreak[2] *infAge[2] * infRace[4], 1 - (infOutbreak[2] *infAge[2] * infRace[4]),
                                            infOutbreak[2] *infAge[3] * infRace[1], 1 - (infOutbreak[2] *infAge[3] * infRace[1]), infOutbreak[2] *infAge[3] * infRace[2], 1 - (infOutbreak[2] *infAge[3] * infRace[2]), infOutbreak[2] *infAge[3] * infRace[3], 1 - (infOutbreak[2] *infAge[3] * infRace[3]), infOutbreak[2] *infAge[3] * infRace[4], 1 - (infOutbreak[2] *infAge[3] * infRace[4]), 
                                            infOutbreak[2] *infAge[4] * infRace[1], 1 - (infOutbreak[2] *infAge[4] * infRace[1]), infOutbreak[2] *infAge[4] * infRace[2], 1 - (infOutbreak[2] *infAge[4] * infRace[2]), infOutbreak[2] *infAge[4] * infRace[3], 1 - (infOutbreak[2] *infAge[4] * infRace[3]), infOutbreak[2] *infAge[4] * infRace[4],1 - (infOutbreak[2] *infAge[4] * infRace[4])), 
                                          dim = c(2, 4, 4, 2), dimnames = list("infection" = c("Infected", "Not_infected"), "race" = c("Asian", "Black", "Latino", "White"), "age" = c("age_18_less", "age_19_o_49", "age_50_to_64", "age_65_up"), "outbreak" = c("No_outbreak", "Yes_outbreak")))
  ...
  return (list(user_community_risk_category, prediction_infected))
